...
namespace Mithril.API.Abstractions.Commands.BaseClasses
{
        ...
        where TCommand : class
    {
        ...
        public ICommandEvent[] HandleCommand(params ICommand[] arg)
        {
            arg ??= Array.Empty<ICommand>();
            var Items = arg.Select(x => x as TCommand).Where(x => x is not null).ToArray();
            if (Items.Length == 0)
                return Array.Empty<ICommandEvent>();
            return HandleCommand(Items!) ?? Array.Empty<ICommandEvent>();
        }
        ...
        protected abstract ICommandEvent[] HandleCommand(params TCommand[] args);
    }
}