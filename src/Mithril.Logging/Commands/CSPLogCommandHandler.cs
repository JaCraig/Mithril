...
namespace Mithril.Logging.Commands
{
    ...
    public class CSPLogCommandHandler(ILogger<CSPLogCommandHandler>? logger, IFeatureManager? featureManager) : CommandHandlerBaseClass<LogCommand, CSPLogCommandVM>(logger, featureManager)
    {
        ...
        public override ValueTask<CommandCreationResult?> CreateAsync(CSPLogCommandVM? value, ClaimsPrincipal user)
        {
        ...
        protected override Task<IEvent[]> HandleCommandAsync(LogCommand?[]? args) => Task.FromResult(Array.Empty<IEvent>());
    }
}