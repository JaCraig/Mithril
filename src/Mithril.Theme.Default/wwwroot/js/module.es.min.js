...
var x = (i, e, t) => (Q(i, typeof e != "symbol" ? e + "" : e, t), t);
import _ from "moment";
import K, { openBlock as S, createElementBlock as D, createTextVNode as X, Fragment as I, renderList as N, createBlock as Z, resolveDynamicComponent as ee, createElementVNode as y, toDisplayString as R, resolveDirective as te, withDirectives as re, withModifiers as se, normalizeClass as V } from "vue";
class ie {
...
class v {
  ...
  minimumLevel(e) {
    return this.filters.push(new ne(e)), this;
  }
  ...
  process(e) {
    this.formatter ?? (this.formatter = new H()), this.sink ?? (this.sink = new B());
...
class oe {
  ...
  write(e, t, r, s) {
    ...
    this.pipelines.forEach((n) => n.process(a));
  }
}
class le {
...
class ce {
  ...
  constructor(e, t, r) {
    ...
    s.onupgradeneeded = (a) => {
      ...
      for (const n of t)
        this.database.objectStoreNames.contains(n) && this.database.deleteObjectStore(n), this.database.createObjectStore(n);
    }, s.onsuccess = (a) => {
      this.database = a.target.result;
    }, s.onerror = (a) => {
      h.error("Failed to open the database:", a.target.error);
    };
  }
  ...
  add(e, t, r) {
    return new Promise((s, a) => {
      ...
      l.onsuccess = () => {
        ...
        h.error("Failed to add an object to the database: ", c.target.error), a(new Error("Failed to add an object to the database: " + c.target.error));
      };
    });
  }
  ...
  getKeys(e) {
    return new Promise((t, r) => {
      const n = this.database.transaction(e, "readwrite").objectStore(e).getAllKeys();
      n.onsuccess = (o) => {
        t(o.target.result);
      }, n.onerror = (o) => {
...
class q {
  // Constructor
  constructor() {
    this.database = new ce("cacheStore", ["cache", "cacheEntryOptions"], 1);
  }
  ...
  async compact() {
    ...
    for (let s = 0; s < e.length; s++) {
      let a = e[s], n = t[s];
      a.expirationTime != 0 && a.expirationTime < r && await this.remove(n.key);
    }
    return this;
  }
}
class F {
  ...
  static configure(e = new q()) {
    this.storageProvider ?? (this.storageProvider = window.StorageProvider || new q()), window.StorageProvider = this.storageProvider;
  }
...
class w {
  ...
  async send() {
    const { authenticationProvider: e, method: t, url: r, data: s, headers: a, credentials: n, serializer: o, parser: l, success: c, error: p, storageMode: u, cacheKey: m, timeout: k, retryAttempts: W, retryDelay: J, retry: G, cancellationToken: A } = this.options, L = new AbortController();
    ...
    const O = async () => {
      ...
      return T < W ? (++T, await new Promise((g) => setTimeout(g, J)), G(T), O()) : (p(P), Promise.reject(P));
    };
    return O();
  }
...
var $ = /* @__PURE__ */ ((i) => (i[i.NetworkFirst = 0] = "NetworkFirst", i[i.StorageFirst = 1] = "StorageFirst", i[i.NetworkOnly = 2] = "NetworkOnly", i[i.StorageAndUpdate = 3] = "StorageAndUpdate", i))($ || {});
class C {
...
class ue {
  ...
  getKeys(e) {
    return e.toUpperCase().split(/-(?!$)/).map((t) => b.keyMappings[t] || t.charCodeAt(0));
  }
...
class ge {
  // constructor
  constructor(e, t) {
    this.keyCodeSequence = e.toUpperCase(), this.keyCodes = e.toUpperCase().split(" ").map((r) => new ue(r)), this.callback = t, this.length = this.keyCodes.length, this.isDefault = e === "<*>";
  }
...
class E {
  ...
  addSequence(e, t) {
    return this.removeSequence(e), this.sequences.push(new ge(e, t)), this;
  }
...
class pe {
  ...
  getKeys(e) {
    let t = [];
    return e.altKey && t.push(b.keyMappings.ALT), e.ctrlKey && t.push(b.keyMappings.CTRL), e.metaKey && t.push(b.keyMappings.META), e.shiftKey && t.push(b.keyMappings.SHIFT), t.indexOf(e.keyCode) === -1 && t.push(e.keyCode), t;
  }
}
class M {
}
...
class we {
  // Constructor
  constructor(e, t, r) {
    r === void 0 && (r = new M()), this.url = this.fixUrl(e), this.pathParts = this.getPathParts(this.url).map((s) => new de(s, r)), this.pathParts = this.pathParts ? this.pathParts : [], this.queryParts = this.getQueryParts(this.url).map((s) => new fe(s, r)), this.queryParts = this.queryParts ? this.queryParts : [], this.hashParts = this.getHashParts(this.url).map((s) => new me(s, r)), this.hashParts = this.hashParts ? this.hashParts : [], this.callbacks = [t], this.defaultValues = r;
  }
...
class ye {
  ...
  addRoute(e, t, r) {
    ...
    return s.length === 0 ? this.routes.push(new we(e, t, r)) : s[0].addCallback(t), this;
  }
...
class Ie {
  ...
  static exportCSV(e, t, r) {
    ...
    for (let c = 0; c < a.length; ++c) {
      let p = a[c], u = t.filter(function(k) {
        return k.property === p;
      });
...
class Se {
  // constructor
  constructor() {
    h.configure().minimumLevel("Debug").enrichWith(new he()).enrichWith(new le()).formatUsing(new H("[{Timestamp}]	[{Level}]	[{caller}]		{Message}{Exception}")).writeTo(new B()), this.localStorage = new ie(), this.sessionStorage = new ae(), this.hotkeys = new pe(), this.router = new ye(), this.history = new be(), window.addEventListener("keydown", (e) => this.hotkeys.press(e)), window.onerror = (e, t, r, s, a) => {
      h.error(e.toString(), { url: t, line: r, column: s, stack: (a == null ? void 0 : a.stack) || "" }, a);
    };
  }
}
let f = {
...
function De(i) {
  return i == null || (i.config.globalProperties.$filters = {
    moment: function(e, t, r) {
      return e == null ? "N/A" : (r ?? (r = "YYYY-MM-DDThh:mm:ss"), t ?? (t = "M-D-YYYY h:mm A"), _(e, r).format(t));
    },
...
const Pe = K.defineComponent({
  ...
  methods: {
    fetchData: function() {
      ...
      }).withStorageMode($.StorageAndUpdate).send();
    },
    saveData: function() {
    },
    deleteData: function() {
...
function Ee(i, e, t, r, s, a) {
  return S(), D("div", null, [
    X(" ASDF "),
    (S(!0), D(I, null, N(i.schema.children, (n) => (S(), Z(ee(n.type), {
      key: n.type,
      data: i.internalData,
      schema: n,
      onSave: i.saveData,
    ...
    y("pre", null, R(i.internalData), 1)
  ]);
}
const Ce = /* @__PURE__ */ z(Pe, [["render", Ee]]), ke = K.defineComponent({
  data: function() {
    return {
      items: this.items,
      showDropDown: !1
...
}), Ae = { class: "unstyled" }, Te = { class: "grouped" }, je = ["href"];
function Fe(i, e, t, r, s, a) {
  ...
  return re((S(), D("ul", Ae, [
    y("li", null, [
      y("a", {
        ...
        href: "#",
        onClick: e[0] || (e[0] = se((...o) => i.showDropDownFunc && i.showDropDownFunc(...o), ["stop", "prevent"]))
      }),
      y("div", {
        class: V({ show: i.showDropDown, "z-depth-2": i.showDropDown, hidden: !i.showDropDown })
      }, [
        y("ul", Te, [
          (S(!0), D(I, null, N(i.items, (o) => (S(), D("li", {
            key: o.display
          }, [
            y("a", {
              href: o.url,
...
const Le = /* @__PURE__ */ z(ke, [["render", Fe]]), Oe = (i) => {
  i.component("mithril-data-handler", Ce), i.component("mithril-side-menu", Le);
}, xe = {
  install: (i, e) => {
    De(i), ve(i), Oe(i);
  }
...
class Ve {
  ...
  setupLogging() {
    h.configure().minimumLevel("Error").writeTo(new qe());
  }
}
class qe {
